...
class MantraAlbumView(APIView):
  ...
  def get(self, request,device_auth,format=None):
    if auth_required(device_auth) == True:
      ...
      albums_serializer = MantraAlbumSerializer(mantra_albums, many=True,context={'device_key': device_auth}
).data
...
class MantraView(APIView):
  ...
  def get(self,request,album_id,device_auth,format=None):
    if auth_required(device_auth) == True:
      ...
      mantras = Mantra.objects.filter(album=album)
      if not mantras:
        return Response({
          "ERROR":"404 NO DATA FOUND :("}, status=status.HTTP_404_NOT_FOUND)
      mantra_serializer = MantraSerializer(songs, many=True,context={'device_key': device_auth}
).data
...
class MantraLyricsView(APIView):
  ...
  def get(self,request,album_id,device_auth,format=None):
    if auth_required(device_auth) == True:
      mantra_album = MantraAlbum.objects.get(id=album_id)
      mantra_lyrics = MantraLyric.objects.filter(album=album)
      if not mantra_lyrics:
        return Response({
          "ERROR":"404 NO DATA FOUND :("}, status=status.HTTP_404_NOT_FOUND)
      mantra_lyrics_serializer = MantraLyricsSerializer(mantra_lyrics, many=True,context={'device_key': device_auth}
).data
      return Response(mantra_lyrics_serializer, status=status.HTTP_200_OK)
    else:
      return Response({"ERROR":"Access Denied"}, status=status.HTTP_404_NOT_FOUND)